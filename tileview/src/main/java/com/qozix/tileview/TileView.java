...
public class TileView extends ZoomPanLayout implements
    ...
    public void slideToAndCenter(double x, double y) {
        slideToAndCenter(mCoordinateTranslater.translateAndScaleX(x, getScale()),
                mCoordinateTranslater.translateAndScaleY(y, getScale())
        );
    }
    ...
    public void moveCallout(View view, double x, double y) {
        mCalloutLayout.moveMarker(view,
                mCoordinateTranslater.translateX(x),
                mCoordinateTranslater.translateY(y));
    }

    public void removeMarkerByTag(Object o) {
        if (o == null) {
            return;
        }
        int childCount = mMarkerLayout.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View childAt = mMarkerLayout.getChildAt(i);
            if (childAt != null) {
                Object tag = childAt.getTag();
                if (o.equals(tag)) {
                    removeMarker(childAt);
                }
            }
        }
    }
    ...
    public boolean onSingleTapConfirmed(MotionEvent event) {
        ...
        double relativeX = mCoordinateTranslater.translateAndScaleAbsoluteToRelativeX(x, getScale());
        double relativeY = mCoordinateTranslater.translateAndScaleAbsoluteToRelativeY(y, getScale());
        if (mOnSingleTapListener != null) {
            mOnSingleTapListener.onSingleTap(relativeX, relativeY);
        }
//        Log.i(TAG, "onSingleTapConfirmed: eventX: " + event.getX() + "  eventY: " + event.getY()
//                + "  ScrollX: " + getScrollX() + "  ScrollY: " + getScrollY()
//                + "  OffsetX: " + getOffsetX() + "  OffsetY: " + getOffsetY()
...
//        );
    ...
    public interface OnSingleTapListener {
        void onSingleTap(double relativeX, double relativeY);
    }
